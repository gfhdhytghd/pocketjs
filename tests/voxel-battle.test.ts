...
import { WildBattle, type BattleButton, type BattleInput, type BattleSave } from "../apps/voxelmon/game/battle/battle.ts";
import { makeBattler } from "../apps/voxelmon/game/battle/battler.ts";
import { newMon, type PartyMon } from "../apps/voxelmon/game/battle/mon.ts";
import { search as arenaSearch } from "../apps/voxelmon/game/battle/arena.ts";
import {
  HUD_BAR_EMPTY,
  HUD_BAR_FULL,
  HUD_BAR_LEFT,
...
import { attempt as catchAttempt } from "../apps/voxelmon/game/rules/catching.ts";
import { compute as damageCompute, GEN1_FAITHFUL } from "../apps/voxelmon/game/rules/damage.ts";
import { gainFor } from "../apps/voxelmon/game/rules/experience.ts";
import { hpBarPixels } from "../apps/voxelmon/game/rules/timing.ts";
import { createTypeChart } from "../apps/voxelmon/game/rules/typechart.ts";
...
async function runBattleTapeInProcess(): Promise<RecorderHost> {
  ...
  game.newGame();
  const tapeText = await Bun.file(join(root, "apps/voxelmon/tapes/battle.tape")).text();
