...
struct Args {
    ...
    hold: u32,
    /// Level-triggered headless input script (`--script "30:16,93:0,120:4096"`):
    /// each `frame:mask[:analog]` entry sets the button mask (and optionally
    /// the packed analog word, spec `(x<<8)|y`, default centered) FROM that
    /// frame until the next entry — the held-movement complement to
    /// tape.ts's pulse `--input`.
    script: Vec<(u32, u32, u32)>,
}

/// spec::ANALOG_CENTER — both axes at 128.
...
fn parse_args() -> Result<Args> {
    ...
    while let Some(a) = it.next() {
        ...
        match a.as_str() {
            ...
            "--script" => {
                for pair in val("--script")?.split(',').filter(|p| !p.is_empty()) {
                    let mut it = pair.split(':');
                    let f = it.next().unwrap_or_default().trim().parse()?;
                    let m = it
                        .next()
                        .ok_or_else(|| anyhow!("--script: '{pair}' is not frame:mask[:analog]"))?
                        .trim()
                        .parse()?;
                    let nub = match it.next() {
                        Some(a) => a.trim().parse()?,
                        None => ANALOG_CENTER,
                    };
                    args.script.push((f, m, nub));
                }
                args.script.sort_by_key(|e| e.0);
            }
...
/// bound node's laid-out rect (engine/core/src/draw.rs). Call once per tick.
...
fn headless(args: &Args) -> Result<()> {
    ...
    let mut nub = ANALOG_CENTER;
    let mut next = 0usize;
    for f in 0..args.frames {
        while next < args.script.len() && args.script[next].0 <= f {
            scripted = args.script[next].1;
            nub = args.script[next].2;
            next += 1;
        }
        guest.frame_with_analog(args.hold | scripted, nub)?;
